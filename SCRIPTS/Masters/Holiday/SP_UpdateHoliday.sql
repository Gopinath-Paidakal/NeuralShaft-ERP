USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateHoliday]    Script Date: 11/05/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateHoliday]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateHoliday]
GO
...
BEGIN TRY
	...
	SET
		BranchId = J.BranchId,
		HolidayDate = J.HolidayDate,
		HolidayName = J.HolidayName,
		HoidayYear = J.HoidayYear,
		HolidayStatus = J.HolidayStatus,
	...
	INNER JOIN OPENJSON(@Holiday, '$.Holiday')
	WITH
	(
	    HolidayId INT,
