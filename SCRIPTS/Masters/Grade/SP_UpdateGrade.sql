USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateGrade]    Script Date: 11/05/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateGrade]') AND type in (N'P', N'PC'))
...
BEGIN TRY
	...
	SET
		...
		GradeDesc = J.GradeDesc,
		GradeStatus = J.GradeStatus,
		ModifiedUserId = J.ModifiedUserId,
		ModifiedDate = J.ModifiedDate
		FROM dbo.Grade G

		INNER JOIN OPENJSON(@Grade, '$.Grade')
		WITH
		(
			GradeId INT,
