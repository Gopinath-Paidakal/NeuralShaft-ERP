USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertGrade]    Script Date: 11/05/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertGrade]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertGrade]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertGrade]    Script Date: 11/05/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_InsertGrade]
(
    @Grade nvarchar(max)
)
----With Encryption
...
BEGIN TRANSACTION;
	...
	FROM OPENJSON(@Grade, '$.Grade' )
	WITH
	(
		GradeCode NVARCHAR(50),
		GradeName NVARCHAR(200),
		GradeDesc NVARCHAR(500),
		GradeStatus BIT,
		CreatedUserId INT,
		CreatedDate DATE
	);
