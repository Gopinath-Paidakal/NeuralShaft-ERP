USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertMenuPermissions]    Script Date: 13/05/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertMenuPermissions]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertMenuPermissions]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertMenuPermissions]    Script Date: 13/05/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_InsertMenuPermissions]
(
...
BEGIN TRANSACTION;
	...
	set @EmpId = JSON_VALUE(@Permissions, '$.Permissions[0].EmpId') 

	set @UserId = (Select UserId from Users where EmpId = @EmpId)

	----------- Deleteing earlier permissions inserting new

		Delete from MenuPermissions where userId = @UserId
	...
	INSERT INTO dbo.MenuPermissions
		...
		SELECT
			...
			CreatedUserId,
			CreatedDate
		FROM OPENJSON(@Permissions,'$.Permissions')
		WITH
		(
			--UserId INT,
			DeptId INT,
