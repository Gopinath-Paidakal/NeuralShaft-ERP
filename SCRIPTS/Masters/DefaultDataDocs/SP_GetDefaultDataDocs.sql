USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_GetDefaultDataDocs]    Script Date: 18/06/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetDefaultDataDocs]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_GetDefaultDataDocs]
GO
...
CREATE PROCEDURE [dbo].[SP_GetDefaultDataDocs]
(
	@DefaultDataDocType nvarchar(100) = ''
...
SET NOCOUNT ON;
BEGIN TRY
DECLARE @DefaultDataDocs   NVARCHAR(MAX)

SET @DefaultDataDocs =
(
    SELECT
        [DefaultDataDocsId],
        ISNULL([DefaultDataDocType], '') AS [DocType],
        ISNULL([DefaulDataDocName], '') AS [DocName]
    FROM DefaultDataDocs
    Where DefaultDataDocType = @DefaultDataDocType

    FOR JSON PATH
)

SELECT JSON_QUERY(@DefaultDataDocs) AS DefaultDataDocs
...
--select @DefaultData
	  ...
	  --or [DefaultDataType] = upper('ClientCategory')
            --or [DefaultDataType] = upper('LeadSource')
            --or [DefaultDataType] = upper('LeadSourceBy')
            --or [DefaultDataType] = upper('Product')
