USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateContact]    Script Date: 04/06/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateContact]') AND type in (N'P', N'PC'))
...
BEGIN TRY
	...
	UPDATE D
		SET
			...
			ContactFirmName = J.ContactFirmName,
			ContactDesignation = J.ContactDesignation,
		...
		INNER JOIN OPENJSON(@Contact,'$.Contact')
		WITH
		(
			ContactId INT,
			ContactType NVARCHAR(100),
			ContactName NVARCHAR(100),
			ContactFirmName NVARCHAR(100),
			ContactDesignation NVARCHAR(100),
			ContactNumber NVARCHAR(100),

			ModifiedUserId INT,
			ModifiedDate DATETIME
		) J
		ON D.ContactId = @ContactId;
