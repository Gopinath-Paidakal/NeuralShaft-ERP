USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateDept]    Script Date: 11/05/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateDept]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateDept]
GO
...
BEGIN TRY
	...
	UPDATE D
		...
		INNER JOIN OPENJSON(@Department,'$.Department')
		WITH
		(
			DeptId INT,
