USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateDeliveryChallanDtl]    Script Date: 30/07/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateDeliveryChallanDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateDeliveryChallanDtl]
GO
...
GO
CREATE PROCEDURE [dbo].[SP_UpdateDeliveryChallanDtl]
(
	@DCDtlId int,	
...
BEGIN TRY
    ...
    UPDATE QDI
        ...
        FROM DeliveryChallanDtl QDI
        INNER JOIN OPENJSON(@DeliveryChallanDtl, '$.DeliveryChallanDtl')
        WITH
        (
            DCDtlId             INT,
