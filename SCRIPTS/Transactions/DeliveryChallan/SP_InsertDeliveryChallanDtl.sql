USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertDeliveryChallanDtl]    Script Date: 31/07/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertDeliveryChallanDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertDeliveryChallanDtl]
GO
...
GO
CREATE PROCEDURE [dbo].[SP_InsertDeliveryChallanDtl]
(
	@DeliveryChallanDtl  nvarchar(Max)
)
----With Encryption
AS
...
BEGIN TRY
       ...
       --set @ModifiedUserId = JSON_VALUE(@DCDtl, '$.ModifiedUserId')
       --set @ModifiedDate = JSON_VALUE(@DCDtl, '$.ModifiedDate')

    BEGIN TRANSACTION

           INSERT INTO DeliveryChallanDtl
            (
                DCHdrId
               ,DCDtlDate
               ,ItemName
            ...
            FROM OPENJSON(@DeliveryChallanDtl,'$.DeliveryChallanDtl')
            WITH
            (
