USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteDeliveryChallanDtl]    Script Date: 30/07/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DeleteDeliveryChallanDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_DeleteDeliveryChallanDtl]
GO
...
GO
CREATE PROCEDURE [dbo].[SP_DeleteDeliveryChallanDtl]
(
	@DCDtlId int
)
----With Encryption
...
BEGIN TRY
       ...
       set @DCHdrId = (select DCHdrId from DeliveryChallanDtl where DCDtlId = @DCDtlId)
