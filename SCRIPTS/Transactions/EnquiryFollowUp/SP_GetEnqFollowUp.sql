USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_GetEnqFollowUp]    Script Date: 30/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetEnqFollowUp]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_GetEnqFollowUp]
GO
...
CREATE PROCEDURE [dbo].[SP_GetEnqFollowUp]
(
...
(
   SELECT
	...
	WHERE [EnqFollowUp].[EnqFollowUpDate] >= @FromDate AND [EnqFollowUp].[EnqFollowUpDate] < DATEADD(DAY, 1, @ToDate)
...
--WHERE [EnqFollowUp].[EnqHdrId] = [EnqHdr].[EnqHdrId] 
