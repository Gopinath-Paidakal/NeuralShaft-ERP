USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateEnqFollowUp]    Script Date: 31/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateEnqFollowUp]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateEnqFollowUp]
GO
...
CREATE PROCEDURE [dbo].[SP_UpdateEnqFollowUp]
(
	@EnqFollowUpId int,
	--@EnqHdrId int,
	@EnqFollowUp nvarchar(Max)
...
BEGIN TRY
	...
	--set @EnqHdrId = JSON_VALUE(@EnqFollowUp, '$.EnqFollowUp.EnqHdrId') 
	...
	BEGIN TRANSACTION
		...
		----======================================
			...
			UPDATE E
			SET
				--E.EnqFollowUpDate       = J.EnqFollowUpDate,
				E.EnqPurpose            = J.EnqPurpose,
				E.EnqLastDiscussion     = J.EnqLastDiscussion,
				E.EnqFollowUpRemarks    = J.EnqFollowUpRemarks,
				E.EnqFollowUpStatus     = J.EnqFollowUpStatus,
				
				E.ModifiedUserId        = J.ModifiedUserId,
				E.ModifiedDate          = j.ModifiedDate

			FROM EnqFollowUp E
			CROSS APPLY OPENJSON(@EnqFollowUp, '$.EnqFollowUp')

			WITH (
				-- EnqFollowUpDate         NVARCHAR(50)  '$.EnqFollowUpDate',
				EnqPurpose              NVARCHAR(500) '$.EnqPurpose',
				EnqLastDiscussion       NVARCHAR(15)  '$.EnqLastDiscussion',
				EnqFollowUpRemarks      NVARCHAR(50)  '$.EnqFollowUpRemarks',
				EnqFollowUpStatus       NVARCHAR(50)  '$.EnqFollowUpStatus',
			
				ModifiedUserId          int           '$.ModifiedUserId',
				ModifiedDate            date          '$.ModifiedDate'
			) J
			WHERE E.EnqFollowUpId = @EnqFollowUpId     --and EnqHdrId = @EnqHdrId 
		...
		FROM OPENJSON(@EnqFollowUp, '$.EnqFollowUp')
		WITH
		(
			EnqNo int,
			EnqHdrId int,
			...
			CreatedUserId int, 
			EnqFollowUpDate date,

			EnqFollowUpRemarks nvarchar(500)
		)

		SET @EnqFollowUpId = SCOPE_IDENTITY()	 

		--================= =================
	
	COMMIT TRANSACTION

	Select @EnqFollowUpId
...
--SELECT ISNULL(SCOPE_IDENTITY(), 0) AS EnqFollowUpId
	...
	--Declare @EnqCabinTypeId int
		...
		--select @EnqNo = max(EnqNo) from EnqHdr where CompanyId = @CompId  and  BranchId = @BranchId
