USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertEnqFollowUp]    Script Date: 31/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertEnqFollowUp]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertEnqFollowUp]
GO
...
CREATE PROCEDURE [dbo].[SP_InsertEnqFollowUp]
(
...
BEGIN TRY
	...
	--BEGIN TRANSACTION
		...
		(
			...
			EnqPurpose nvarchar(100),
			EnqLastDiscussion nvarchar(100),
			EnqFollowUpRemarks nvarchar(500),
			EnqFollowUpStatus nvarchar(50),
			CreatedUserId int
