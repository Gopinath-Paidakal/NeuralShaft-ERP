USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertQuoteSOItemDtl]    Script Date: 27/07//20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertQuoteSOItemDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertQuoteSOItemDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertQuoteSOItemDtl]    Script Date: 27/07//2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_InsertQuoteSOItemDtl]
(
	@QuoteSOItemDtl  nvarchar(Max)
)
----With Encryption
AS
...
BEGIN TRY
    ...
    BEGIN TRANSACTION
           ...
           UPDATE H
                SET
                    H.QuoteSOItemAmount               = ISNULL(T.ItemAmount,0),
                    H.QuoteSOItemDiscountAmount       = ISNULL(T.ItemDiscountAmount,0),
                    H.QuoteSOItemTaxAmount            = ISNULL(T.ItemTaxAmount,0),
                    H.QuoteSOItemTotalAmount      = ISNULL(T.ItemTotalAmount,0)
                ...
                (
                    ...
                    FROM QuoteSOItemDtl D
                    WHERE D.QuoteSOItemHdrId =  @QuoteSOItemHdrId  -- H.ItemQuoteHdrId
                ) T
