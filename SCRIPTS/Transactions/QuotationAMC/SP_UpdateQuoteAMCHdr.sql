USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateQuoteAMCHdr]    Script Date: 01/07/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateQuoteAMCHdr]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateQuoteAMCHdr]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateQuoteAMCHdr]    Script Date: 01/07/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_UpdateQuoteAMCHdr]
(
	@QuoteAMCHdrId int,
...
BEGIN TRY
    ...
    ------------------------------------------------
	 UPDATE QAH
        SET
              --QAH.CompanyId             = J.CompanyId,
              --QAH.BranchId              = J.BranchId,
              --QAH.OrdClientHdrId        = J.OrdClientHdrId,
              --QAH.QuoteAMCNo            = J.QuoteAMCNo,
              --QAH.QuoteAMCSlNo          = J.QuoteAMCSlNo,
              --QAH.QuoteAMCDate          = J.QuoteAMCDate,
        ...
        FROM QuoteAMCHdr QAH
        INNER JOIN OPENJSON(@QuoteAMCHdr, '$.QuoteAMCHdr')
        WITH
        (
              QuoteAMCHdrId             INT,
              --CompanyId                 INT,
        ...
        ON QAH.QuoteAMCHdrId = J.QuoteAMCHdrId;
