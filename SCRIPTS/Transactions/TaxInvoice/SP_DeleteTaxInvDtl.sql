USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteTaxInvDtl]    Script Date: 30/07/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DeleteTaxInvDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_DeleteTaxInvDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteTaxInvDtl]    Script Date: 30/07/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_DeleteTaxInvDtl]
(
	@TaxInvDtlId int
)
----With Encryption
...
BEGIN TRY
       ...
       set @TaxInvHdrId = (select TaxInvHdrId from TaxInvDtl where TaxInvDtlId = @TaxInvDtlId)
      

	BEGIN TRANSACTION

            Delete from TaxInvDtl where TaxInvDtlId = @TaxInvDtlId
