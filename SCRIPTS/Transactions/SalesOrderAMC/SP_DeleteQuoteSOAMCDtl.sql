USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteQuoteSOAMCDtl]    Script Date: 27/07//20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DeleteQuoteSOAMCDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_DeleteQuoteSOAMCDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteQuoteSOAMCDtl]    Script Date: 27/07//2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_DeleteQuoteSOAMCDtl]
(
	@QuoteSOAMCDtlId int
)
----With Encryption
...
BEGIN TRY
       ...
       set @QuoteSOAMCHdrId = (select QuoteSOAMCHdrId from QuoteSOAMCDtl where  QuoteSOAMCHdrId = @QuoteSOAMCHdrId)
