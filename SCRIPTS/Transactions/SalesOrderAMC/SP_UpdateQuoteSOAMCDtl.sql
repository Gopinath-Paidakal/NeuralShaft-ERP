USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateQuoteSOAMCDtl]    Script Date: 27/07//2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateQuoteSOAMCDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateQuoteSOAMCDtl]
GO
...
GO
CREATE PROCEDURE [dbo].[SP_UpdateQuoteSOAMCDtl]
(
	@QuoteSOAMCDtlId int,	
...
BEGIN TRY
    ...
    UPDATE QDI
        ...
        INNER JOIN OPENJSON(@QuoteSOAMCDtl, '$.QuoteSOAMCDtl')
        WITH
        (
            QuoteItemDtlId             INT,
            --ItemQuoteHdrId             INT,
