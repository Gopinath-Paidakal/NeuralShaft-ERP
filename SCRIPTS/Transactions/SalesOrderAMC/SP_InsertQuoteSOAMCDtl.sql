USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertQuoteSOAMCDtl]    Script Date: 27/07//20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertQuoteSOAMCDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertQuoteSOAMCDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertQuoteSOAMCDtl]    Script Date: 27/07//2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_InsertQuoteSOAMCDtl]
(
	@QuoteSOAMCDtl  nvarchar(Max)
)
----With Encryption
AS
...
BEGIN TRY
       ...
       set @QuoteSOAMCHdrId = JSON_VALUE(@QuoteSOAMCDtl, '$.QuoteSOAMCHdrId')
       set @ModifiedUserId = JSON_VALUE(@QuoteSOAMCDtl, '$.ModifiedUserId')
       set @ModifiedDate = JSON_VALUE(@QuoteSOAMCDtl, '$.ModifiedDate')
    ...
    BEGIN TRANSACTION
           ...
           INSERT INTO QuoteSOAMCDtl
            ...
            FROM OPENJSON(@QuoteSOAMCDtl,'$.QuoteSOAMCDtl')
            WITH
            (
