USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_GetReceipt]    Script Date: 05/08/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetReceipt]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_GetReceipt]
GO

USE [NSERPLIVE]
GO

SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_GetReceipt]
(
	@FromDate nvarchar(20),
	@ToDate nvarchar(20)
)

AS

SET NOCOUNT ON;

BEGIN TRY
	BEGIN TRANSACTION

	-- GET ALL BRANCHES AS JSON
		  SELECT
		(

		SELECT [ReceiptHdrId]
			  --,[CompanyId]
			  --,[BranchId]
			  ,[ReceiptHdr].[OrdClientHdrId]
			  ,[OrdClientHdr].[OrdClientName]
			  ,[ReceiptNo]
			  ,[ReceiptDate]

			  ,[PaymentType]
			  ,[ChequeDeposited]
			  ,[BankName]
			  ,[ChequeNo]
			  ,[ChequeAmount]
			  ,[ChequeStatus]
			  ,[PaymentDetails]
			  ,[Amount]
			  ,[TotalAmount]
     
			FROM [dbo].[ReceiptHdr]
			
			INNER JOIN [OrdClientHdr] ON [OrdClientHdr].[OrdClientHdrId] = [ReceiptHdr].[OrdClientHdrId]

			--- Both Date and Time-- best practice
			WHERE [ReceiptHdr].[ReceiptDate] >= @FromDate AND [ReceiptHdr].[ReceiptDate] < DATEADD(DAY, 1, @ToDate)

			Order by [ReceiptHdr].[ReceiptNo]

		
			FOR JSON PATH, ROOT('Receipt')  -- ROOT WITHOUT_ARRAY_WRAPPER

		) AS Receipt

	COMMIT TRANSACTION
END TRY

	BEGIN CATCH
		ROLLBACK TRANSACTION
		Declare 
		@ErrMsg varchar(4000),
		@ErrSeverity int,
		@ErrProcedure varchar(100)

		SET @ErrMsg = (Select Error_Message())
		SET @ErrSeverity = (Select Error_Severity())
		SET @ErrProcedure = (Select Error_Procedure())

		SET @ErrMsg = @ErrMsg + ' / ' + @ErrProcedure
		Raiserror(@ErrMsg,@ErrSeverity,1)
		GOTO End_Prog

	END CATCH

End_Prog:

 --,[CreatedUserId]
 --     ,[CreatedDate]
 --     ,[ModifiedUserId]
 --     ,[ModifiedDate]