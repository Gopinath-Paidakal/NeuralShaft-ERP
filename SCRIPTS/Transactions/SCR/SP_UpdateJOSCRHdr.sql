USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOSCRHdr]    Script Date: 25/06/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateJOSCRHdr]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateJOSCRHdr]
GO
...
CREATE PROCEDURE [dbo].[SP_UpdateJOSCRHdr]
(
	@JobOrderSCRHdrId int,	
...
BEGIN TRY
	...
	UPDATE T
		SET
			...
			T.SpaceForMatlStatus  = J.SpaceForMatlStatus,

			T.Remarks             = J.Remarks,
			T.SCRDocPath          = J.SCRDocPath,
			T.SCRDocName          = J.SCRDocName

		FROM dbo.JobOrderSCRHdr T
		CROSS APPLY OPENJSON(@JobOrderSCRHdr, '$.JobOrderSCRHdr')
		WITH
		(
			Id                  INT,
			Pit                 INT,
		...
		WHERE T.JobOrderSCRHdrId = JobOrderSCRHdrId     --J.Id;
        
	
		---------------------------------------------------------
		--- UPdating Detail JobOrderSCRDtl
		-----====================================================

		UPDATE D
			SET
				D.[Floor] = J.[Floor],
				D.[LeftWall] = J.[LeftWall],
				D.[RightWall] = J.[RightWall],
				D.[FFLMarking] = J.[FFLMarking]

				--D.ModifiedUserId = J.ModifiedUserId,
				--D.ModifiedDate = J.ModifiedDate

			FROM JobOrderSCRDtl D
			INNER JOIN OPENJSON(@JobOrderSCRHdr, '$.JobOrderSCRDtl')
			WITH
			(
				JobOrderSCRDtlId INT,
			...
			ON D.JobOrderSCRDtlId = J.JobOrderSCRDtlId;
		

		Select @JobOrderSCRHdrId
