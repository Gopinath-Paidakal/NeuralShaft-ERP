USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOSVRDtl]    Script Date: 24/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateJOSVRDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateJOSVRDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOSVRDtl]    Script Date: /03/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_UpdateJOSVRDtl]
(
	@JobOrderSVRDtlId int,	
	@JobOrderSVRDtl nvarchar(Max)
...
BEGIN TRY
	...
	UPDATE D
		...
		(
			SELECT *
			FROM OPENJSON(@JobOrderSVRDtl, '$.JobOrderSVRDtl')
			WITH
			(
				JobOrderSVRDtlId INT,
