USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOSVR_NewFile]    Script Date: 24/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateJOSVR_NewFile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateJOSVR_NewFile]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOSVR_NewFile]    Script Date: /03/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_UpdateJOSVR_NewFile]
(
...
BEGIN TRY
	BEGIN TRANSACTION
		...
		Update JobOrderSCRHdr set SCRDocName = @NewFileName where JobOrderSCRHdrId = @JobOrderSCRHdrId
