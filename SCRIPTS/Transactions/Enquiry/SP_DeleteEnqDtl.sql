USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteEnqDtl]    Script Date: 24/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DeleteEnqDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_DeleteEnqDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteEnqDtl]    Script Date: /03/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_DeleteEnqDtl]
(
	@EnqDtlId int
	--@EnqDtl nvarchar(Max)
...
BEGIN TRY
	BEGIN TRANSACTION
	------------------------------------------------
	-- DELETE - Setting the Delete flag with 1 - True
	------------------------------------------------
		UPDATE EnqDtl SET Deleted = 1 where EnqDtlId = @EnqDtlId;

	    
	    Select * from EnqDtl where EnqDtlId = @EnqDtlId and Deleted = 0
