USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateEnqHdr]    Script Date: 24/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateEnqHdr]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateEnqHdr]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateEnqHdr]    Script Date: /03/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_UpdateEnqHdr]
(
	@EnqHdrId int,	
	@EnqHdr nvarchar(Max)
...
BEGIN TRY
    ...
    SET
        ...
        E.ModifiedUserId   = J.ModifiedUserId,   -- -1,     ----J.ModifiedBy,
     ...
     ---   Update Client Details
      --------------------------------
		...
		SET
			...
			C.EnqClientSalutation = J.EnqClientSalutation,
	...
	------------------------------------------------
    -- UPDATE ENQUIRY PHOTO
    ------------------------------------------------
	--UPDATE P
 --   SET
 --       P.EnqPhotoName = J.EnqPhotoName,
 --       P.EnqPhotoRelPath    = J.EnqPhotoRelPath,
 --       P.ModifiedBy    = 'Admin',    
	--	P.ModifiedDate  = GETDATE()
 --   FROM EnqPhoto P
 --   CROSS APPLY OPENJSON(@EnqHdr, '$.EnqPhoto')
 --   WITH (
 --       EnqPhotoName     NVARCHAR(100) '$.EnqPhotoName',
 --       EnqPhotoRelPath  NVARCHAR(500) '$.EnqPhotoRelPath'
 --       --ModifiedBy    NVARCHAR(50)  '$.ModifiedBy'
 --   ) J
 --   WHERE E.EnqHdrId = @EnqHdrId
