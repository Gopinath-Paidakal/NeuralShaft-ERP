...
BEGIN TRY
	...
	set @PassengerAmount = (Select PassengerAmount from EnqDtl where EnqDtlId = @EnqDtlId)
	...
	update EnqDtl set EnqRate = @EnqProductAmount, EnqProductAmount = @EnqProductAmount, EnqSubTotal = @EnqProductAmount, 
        ...
        --  Extract EnqDtlId
        ---------------------------------------------------
		SELECT @EnqDtlId = EnqDtlId FROM OPENJSON(@EnqDtl) WITH (EnqDtlId INT);

		---------------------------------------------------
		-- LAND DOOR MERGE (Handles Insert, Update, & Delete)
		---------------------------------------------------
		MERGE INTO EnqLandDoor AS Target
		USING (
			SELECT 
				EnqLandDoorId,
				-- Fallback to parent ID if the array item misses it
				ISNULL(EnqDtlId, @EnqDtlId) AS EnqDtlId, 
				LandFloorType,
				LandDoorType,
				LandDoorFinishType,
				LandDoorAngle,
				...
				LandDoorDescription,
				LandDoorAmount,
				UPPER(CrudType) AS CrudType -- Normalize case early
		...
		WHEN MATCHED AND Source.CrudType = 'DELETE' THEN
			DELETE
		...
		WHEN MATCHED THEN
			UPDATE SET
				...
				Target.CrudType            = Source.CrudType

		-- 3. Action: INSERT (If new record and not flagged as a phantom delete)
		WHEN NOT MATCHED BY TARGET AND Source.CrudType <> 'DELETE' THEN
			INSERT (
				EnqDtlId,
				LandFloorType,
				LandDoorType,
		...
		---------------------------------------------------
		-- CAR DOOR MERGE (Handles Insert, Update, & Delete)
		---------------------------------------------------
