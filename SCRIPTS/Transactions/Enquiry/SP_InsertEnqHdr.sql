USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertEnqHdr]    Script Date: 09/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertEnqHdr]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertEnqHdr]
GO
...
CREATE PROCEDURE [dbo].[SP_InsertEnqHdr]
(
	@EnqHdr nvarchar(Max)
	--@EnqDtl nvarchar(Max)
...
BEGIN TRY
	...
	SELECT
		 ...
		 @EnqNo,
		 @EnqSlNo,
		 --dbo.fn_FormatDateTime([CreatedDate]) as 'CreatedDate',
		 CreatedDate,
	...
	(
		...
		--CreatedTime nvarchar(20)
	)

	SET @EnqHdrId = SCOPE_IDENTITY()	 
	--================= =================
    ...
    (
		   ...
		   EnqClientSalutation nvarchar(15),
	...
	SET @EnqClientId = SCOPE_IDENTITY()	 
	...
	INSERT INTO [dbo].[EnqFollowUp]
		...
		SELECT
			 ...
			 CreatedDate,
			 --dbo.fn_FormatDateTime([CreatedDate]) + 5,
			 --EnqStatus,
			 
			 CreatedUserId,
			 --dbo.fn_FormatDateTime([CreatedDate])
			 CreatedDate
...
----============================================
 ...
 --   FROM OPENJSON(@EnqHdr, '$.EnqPhoto')
 --   WITH
 --   (
	--	   EnqPhotoName nvarchar(100),
	--	   EnqPhotoRelPath nvarchar(200)
 --   )
