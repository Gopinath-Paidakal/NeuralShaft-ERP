USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOPTCDtl]    Script Date: 24/03/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateJOPTCDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateJOPTCDtl]
GO

USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOPTCDtl]    Script Date: /03/2026  ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_UpdateJOPTCDtl]
(
	@JobOrderPTCDtlId int,	
...
BEGIN TRY
	BEGIN TRANSACTION
	 ...
	 UPDATE T
		SET
			--T.JobOrderSVRHdrId = J.JobOrderSVRHdrId,
			T.Width = J.Width,
			T.Depth = J.Depth,
			T.Pit = J.Pit,
			T.Ovehead = J.Ovehead,
			...
			T.CarTop = J.CarTop,
			T.CarBottom = J.CarBottom,
			T.CouterTop = J.CouterTop,
			T.CounterBottom = J.CounterBottom

		FROM dbo.JobOrderPTCDtl T
		INNER JOIN OPENJSON(@JobOrderPTCDtl, '$.JobOrderPTCDtl')
		WITH
		(
			--JobOrderPTCId INT,
        ...
        ON T.JobOrderPTCDtlId = @JobOrderPTCDtlId    ---J.JobOrderPTCDtlId;
