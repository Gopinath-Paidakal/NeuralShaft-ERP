USE [NSERPLIVE]
GO
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetOrdClientByIdSODtl]') AND type IN (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_GetOrdClientByIdSODtl]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_GetOrdClientByIdSODtl]
(
    @SOHdrId INT
)
----With Encryption
...
DECLARE @ProformaInvSODtl NVARCHAR(MAX)
BEGIN TRY
    ...
    SET @SODtl = (
      SELECT [SODtlId]
          ...
          FROM [dbo].[SoDtl]
            ...
            WHERE SOHdr.SOHdrId = @SOHdrId
        FOR JSON PATH
    )


    SET @ProformaInvSODtl = (
        SELECT
