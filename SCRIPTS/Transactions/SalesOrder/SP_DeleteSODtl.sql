USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteSODtl]    Script Date: 11/04/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DeleteSODtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_DeleteSODtl]
GO
...
CREATE PROCEDURE [dbo].[SP_DeleteSODtl]
(
	@SODtlId int
	--@EnqDtl nvarchar(Max)
...
BEGIN TRY
	...
	------------------------------------------------
		UPDATE SODtl SET Deleted = 1 where SODtlId = @SODtlId;

	    
	    Select * from SODtl where SODtlId = @SODtlId and Deleted = 0
