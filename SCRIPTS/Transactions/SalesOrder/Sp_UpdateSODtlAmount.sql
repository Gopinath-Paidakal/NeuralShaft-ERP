USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[Sp_UpdateSODtlAmount]    Script Date: 28/05/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Sp_UpdateSODtlAmount]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Sp_UpdateSODtlAmount]
GO
...
CREATE PROCEDURE [dbo].[Sp_UpdateSODtlAmount]
(	
	@SODtlId int,
	@SODtl nvarchar(Max)
...
BEGIN TRY
	...
	BEGIN TRANSACTION
	--Declare @SOQty numeric(18,2)
	--Declare @SORate numeric(18,2)
	...
	--Declare @SOSubTotal numeric(18,2)
	...
	----- Total Amount
	--Declare @SOTotalAmount numeric(18,2)
	--Declare @SOGrandTotal numeric(18,2)
	...
	------- Product Amount
	--set @SOQty = JSON_VALUE(@SODtl, '$.SODtlAmount.SOQty') 
	--set @SORate = JSON_VALUE(@SODtl, '$.SODtlAmount.SORate') 
	--set @SOProductAmount = JSON_VALUE(@SODtl, '$.SODtlAmount.SOProductAmount') 

	--set @SOProductAmount = (@SOQty * @SORate)

	------ Increase Percentage & IncreaseAmount
	--set @IncreaseAmtPercentage = JSON_VALUE(@SODtl, '$.SODtlAmount.IncreasePercentage')
	--set @IncreaseAmt = (@SOProductAmount * @IncreaseAmtPercentage)/100
	--set @TotIncreaseAmount = @SOProductAmount + @IncreaseAmt

	---- Discount Percentage and Discount Amount
	--set @DiscountPercentage = JSON_VALUE(@SODtl, '$.SODtlAmount.DiscountPercentage')	
	--set @DiscountAmt = (@TotIncreaseAmount  * @DiscountPercentage)/100	
	--set @TotDiscountAmount = (@TotIncreaseAmount - @DiscountAmt)

	-------- Sub Total
	--set @SOSubTotal = (@SOProductAmount - @TotDiscountAmount)

	-------  Finally Taxable Value, and Tax has to be calculated and Total Amount

	----- Taxable value 
	--set @TaxableValue = JSON_VALUE(@SODtl, '$.SODtlAmount.TaxableValue')
	--set @TaxableValueAmount = (@SOSubTotal * @TaxableValue)/100

	----- Tax Calculation
	--set @TaxPerGST = JSON_VALUE(@SODtl, '$.SODtlAmount.GST')
	--set @TaxAmount = (@TaxableValue * @TaxPerGST) / 100

	----- Total Amount
	--set @SOTotalAmount = @SOSubTotal + @TaxAmount
	--set @SOGrandTotal = @SOTotalAmount
	
	----- AMC Percentage and AMCTotal
	--set @AMCPercentage = JSON_VALUE(@SODtl, '$.SODtlAmount.AMCPercentage') 
	--set @AMCAmount = JSON_VALUE(@SODtl, '$.SODtlAmount.AMCAmount')

	--set @GST = JSON_VALUE(@SODtl, '$.SODtlAmount.GST') 
	--------------------------------------------------
	---- UPDATE SODTL Amounts
	----------------------------------------------------
	--Update SODtl 
	--	set SOQty = @SOQty,
	--		SORate = @SORate,
	--		SOProductAmount = @SOProductAmount,
	...
	--		SOTaxAmount = @TaxAmount,

	--		SOTotalAmount = @SOTotalAmount,
	--		SOGrandTotal = @SOGrandTotal
	...
	--------------------------------------------------
		...
		SET
			...
			E.[SOProductAmount] = J.[SOProductAmount],
			...
			E.[SOSubTotal] = J.[SOSubTotal],
			E.[SOTaxAmount] = J.[SOTaxAmount],
			E.[SOTotalAmount] = J.[SOTotalAmount],
			E.[SOGrandTotal] = J.[SOTotalAmount]

		FROM [dbo].[SODtl] E

		CROSS APPLY OPENJSON(@SODtl, '$.SODtlAmount')
		WITH
		(
			...
			[SOTaxAmount] DECIMAL(18,2),
			[SOTotalAmount] DECIMAL(18,2),
			[SOGrandTotal] DECIMAL(18,2)

		) J

		WHERE E.SODtlId = @SODtlId
		---===================================================
