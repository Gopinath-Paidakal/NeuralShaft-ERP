USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_GetSOHdr_ById]    Script Date: 11/04/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetSOHdr_ById]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_GetSOHdr_ById]
GO
...
CREATE PROCEDURE [dbo].[SP_GetSOHdr_ById]
(
	@SOHdrId int
    --@SODtlId int
...
BEGIN TRY
        ...
        set @OrdClientHdrId = (Select OrdClientHdrId from SOHdr where SOHdrId = @SOHdrId)
        ...
        (
	        ...
	        SELECT 
              ...
              ,[SOHNo]
              --,[SOHDate]
              ,FORMAT(SOHDate, 'dd-MM-yyyy') as SOHDate    -- + ' ' + FORMAT(SOHDate, 'HH:mm:ss') 
              ,[SOConsultant]
              ,[SOCustComp]
