USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdatePurchaseOrderDtl]    Script Date: 30/07/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdatePurchaseOrderDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdatePurchaseOrderDtl]
GO
...
GO
CREATE PROCEDURE [dbo].[SP_UpdatePurchaseOrderDtl]
(
...
BEGIN TRY
    ...
    UPDATE QDI
        ...
        FROM PurchaseOrderDtl QDI
        INNER JOIN OPENJSON(@PurchaseOrderDtl, '$.PurchaseOrderDtl')
        WITH
        (
            PurchaseOrderDtlId             INT,
        ...
        ON QDI.PurchaseOrderDtlId = J.PurchaseOrderDtlId;
