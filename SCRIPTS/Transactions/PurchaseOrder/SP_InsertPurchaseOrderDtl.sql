USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_InsertPurchaseOrderDtl]    Script Date: 30/07/20266 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_InsertPurchaseOrderDtl]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_InsertPurchaseOrderDtl]
GO
...
GO
CREATE PROCEDURE [dbo].[SP_InsertPurchaseOrderDtl]
(
	@PurchaseOrderDtl  nvarchar(Max)
)
----With Encryption
AS
...
BEGIN TRY
       ...
       Declare @PurchaseOrderDtlId int
       Declare @PurchaseOrderHdrId int
       --Declare @ModifiedUserId int
       --Declare @ModifiedDate date

       set @PurchaseOrderHdrId = JSON_VALUE(@PurchaseOrderDtl, '$.PurchaseOrderDtl.PurchaseOrderHdrId')
       --set @ModifiedUserId = JSON_VALUE(@PurchaseOrderDtl, '$.ModifiedUserId')
       --set @ModifiedDate = JSON_VALUE(@PurchaseOrderDtl, '$.ModifiedDate')

    BEGIN TRANSACTION

           INSERT INTO PurchaseOrderDtl
            (
                PurchaseOrderHdrId
            ...
            FROM OPENJSON(@PurchaseOrderDtl,'$.PurchaseOrderDtl')
            WITH
            (
            ...
            Set @PurchaseOrderDtlId = SCOPE_IDENTITY()

        ------------------------------------------------------------------
        ------ Updating the totals in PO Hdr
        ------------------------------------------------------------------
