USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_UpdateJOBOMItem]    Script Date: 11/05/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_UpdateJOBOMItem]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_UpdateJOBOMItem]
GO
...
CREATE PROCEDURE [dbo].[SP_UpdateJOBOMItem]
(
	@JobOrderBOMId int,
	@Qty int	
)
...
BEGIN TRY
		...
		Update JobOrderBOM set ItemTotalQty = @Qty where JobOrderBOMId = @JobOrderBOMId
