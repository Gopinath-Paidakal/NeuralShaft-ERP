USE [NSERPLIVE]
GO
/****** Object:  StoredProcedure [dbo].[SP_DeleteJOBOMItem]    Script Date: 07/09/2026 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_DeleteJOBOMItem]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_DeleteJOBOMItem]
GO
...
CREATE PROCEDURE [dbo].[SP_DeleteJOBOMItem]
(
	@JobOrderBOMId int
)
----With Encryption
...
BEGIN TRY
	  ...
	  where JobOrderBOMId = @JobOrderBOMId
