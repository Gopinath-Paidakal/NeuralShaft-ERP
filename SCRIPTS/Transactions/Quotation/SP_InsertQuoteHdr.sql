...
BEGIN TRY
    ...
    set @Prefix = (select DefaultDataName from DefaultData where FormType = 'Quotation' and DefaultDataType = 'Prefix' and DefaultDataOrderBy = 1 )
    ...
    INSERT INTO [dbo].[QuoteHdr]
     ...
     VALUES
           ...
           ,(Select EnqClientSalutation from EnqClient where EnqHdrId = @EnqHdrId)
...
End_Prog:
   ...
   --        -----------------------------------------------------
   --        ----- Updating Quotation detail with QuoteHeader Id
   --        ---------------------------------------------------------
   --        Update QuoteDtl set QuoteHdrId = @QuoteHdrId where QuoteHdrId = @QuoteHdrId 
